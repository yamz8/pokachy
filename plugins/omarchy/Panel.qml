...
Panel {
  ...
  function refresh() {
    if (!statusProc.running) {
      statusProc.command = ["pokachy", "status", "--json"]
  ...
  function runAction(label, args) {
    ...
    actionError = ""
    actionLabel = label
    actionProc.command = ["pokachy"].concat(args)
  ...
  Process {
    id: watchProc
    running: true
    command: ["pokachy", "watch", "--json"]
    stdout: SplitParser { onRead: function(line) { root.parseState(line) } }
    stderr: SplitParser { onRead: function(line) { root.statusError = String(line).trim() || "Pokachy watch stopped." } }
  ...
  Process {
    ...
    command: []
    stdout: SplitParser { onRead: function(line) { root.parseState(line) } }
    stderr: SplitParser { onRead: function(line) { root.statusError = String(line).trim() } }
      ...
      Flickable {
        ...
        Column {
          ...
          Column {
            ...
            Button { text: "Open setup"; onClicked: root.setup() }
          }
          ...
          Column {
            ...
            Row {
              ...
              Button { id: quietButton; text: root.pokachyState.me && root.pokachyState.me.quiet ? "Resume" : "Quiet"; onClicked: root.runAction("Updating quiet mode…", ["quiet", root.pokachyState.me && root.pokachyState.me.quiet ? "off" : "on"]) }
            }
            ...
            Row {
              width: parent.width
              spacing: Style.space(8)
              TextField { id: friendField; width: Math.max(Style.space(180), parent.width - addButton.implicitWidth - Style.space(8)); placeholderText: "Add @handle"; onAccepted: root.addFriend() }
              Button { id: addButton; text: "Add"; onClicked: root.addFriend() }
            }
  ...
  Component {
    ...
    Item {
      ...
      Row { id: rowActions; anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; spacing: Style.space(4)
        Button { text: "Back"; onClicked: root.runAction("Sending poke…", ["poke", "@" + root.handle(modelData)]) }
        Button { text: "Dismiss"; onClicked: root.runAction("Dismissing poke…", ["dismiss", String(modelData.id)]) }
        PanelActionButton { iconText: "󰅖"; foreground: root.foreground; hoverColor: root.bar ? root.bar.urgent : Color.urgent; tooltipText: "Block"; onClicked: root.runAction("Blocking…", ["block", "@" + root.handle(modelData)]) }
      }
    }
  }
  Component {
    ...
    Item {
      ...
      Text { id: name; anchors.left: parent.left; anchors.verticalCenter: parent.verticalCenter; width: Math.max(Style.space(120), parent.width - rowActions.implicitWidth - Style.space(8)); text: root.displayName(modelData); textFormat: Text.PlainText; elide: Text.ElideRight; color: root.foreground; font.family: root.fontFamily; font.pixelSize: Style.font.body }
      Row { id: rowActions; anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; spacing: Style.space(4)
        Button { text: "Poke"; onClicked: root.runAction("Sending poke…", ["poke", "@" + root.handle(modelData)]) }
        PanelActionButton { iconText: "󰆴"; foreground: root.foreground; hoverColor: root.bar ? root.bar.urgent : Color.urgent; tooltipText: "Remove friend"; onClicked: root.runAction("Removing friend…", ["friends", "remove", "@" + root.handle(modelData)]) }
        PanelActionButton { iconText: "󰅖"; foreground: root.foreground; hoverColor: root.bar ? root.bar.urgent : Color.urgent; tooltipText: "Block"; onClicked: root.runAction("Blocking…", ["block", "@" + root.handle(modelData)]) }
      }
    }
  }
  Component {
    ...
    Item {
      ...
      Row { id: requestActions; anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; spacing: Style.space(4)
        Button { visible: modelData.outgoing !== 1; text: "Accept"; onClicked: root.runAction("Accepting request…", ["friends", "accept", "@" + root.handle(modelData)]) }
        Button { text: modelData.outgoing === 1 ? "Cancel" : "Decline"; onClicked: root.runAction(modelData.outgoing === 1 ? "Canceling request…" : "Declining request…", ["friends", "remove", "@" + root.handle(modelData)]) }
      }
    }
  }
  Component {
    ...
    Item {
      ...
      Button { id: action; anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; text: "Unblock"; onClicked: root.runAction("Unblocking…", ["unblock", "@" + root.handle(modelData)]) }
    }
  }
}

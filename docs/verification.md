...
The native fixture sends read-only health requests while awaiting device approval. Wrangler 4.129.1's `ProxyWorker.ts` documents an internal workerd connection-close race at five idle seconds, coinciding with device polling; this keeps the disposable development proxy active. The fixture does not retry token redemption, change polling rules, or weaken production authentication. Transport failures report only fixed categories, plus Worker health and credential-file presence, never raw tokens or device URLs.
